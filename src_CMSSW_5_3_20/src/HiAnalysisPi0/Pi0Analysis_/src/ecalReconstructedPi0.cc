...
#include "HiAnalysisPi0/Pi0Analysis_/interface/ecalReconstructedPi0.h"
...
ecalReconstructedPi0::ecalReconstructedPi0(const edm::ParameterSet& ps):
  nMassBins(200),
  clustEnrCut(0.4),
  clustS49Cut(0.74),
  clustPtCut(0.0),
  ...
  bkgFactor_(10)
{
  ...
  //maximumPi0MassForHistogram(0.30),
  maximumPi0MassForHistogram = ps.getParameter<double>("maximumPi0MassForHistogram");
  ...
  cout << "INSIDE PARAMETER SET" << endl;
...
// Closing - ParameterSet

ecalReconstructedPi0::~ecalReconstructedPi0()
{
...
void
ecalReconstructedPi0::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	int c = 0; 
	cout << c++ << " " ;
...
//  if(occByCentrality_) occ = (double) centProvider->getBin();
  ...
  nClu = 0;
    ...
    std::sort( vsorted.begin(), vsorted.end(),ecalReconstructedPi0::vtxSort );
	...
	if(iPrintEtaCorrection < nPrintEtaCorrection) {
	  cout << "\n First pass: zVertexEventSelected = " << 0.01*zVertexEventSelected << endl;  // convert to meters
	  cout << ", zAtECal = " << zAtECal << endl;
	  cout << ", zCorrected = " << zCorrected << endl;
	  cout << ", thetaForZEqualsZero = " << thetaForZEqualsZero << endl;
	  cout << ", thetaCorrected = " << thetaCorrected << endl;
	  cout << ", zCorrectedEta = " << zCorrectedEta << endl;
	  cout << ", uncorrected Eta = " << etaForZEqualsZero << endl;
...
// Testing - Ravi - BEGIn
void myClass::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{

	cout << "Inside myClass analyze  method" << endl;
}
void myClass::endJob(){}
*/
// Testing - Ravi - End
...
void ecalReconstructedPi0::beginJob() 
{
  ...
  cout << "Inside beginJob Method" << endl;
...
void ecalReconstructedPi0::endJob() 
{

//  cout << "Inside endJob() method" << endl;
  ...
  cout << "\n\n ecalReconstructedPi0::endJob() <I>  nevttotal_trg = " << nevttotal_trg;
  cout << ",  nevttotal_ass = " << nevttotal_ass << endl;
  cout << "\n Will check the Z-vertex differences" << endl;
  cout << endl;
...
void ecalReconstructedPi0::initHistos(const edm::Service<TFileService> & fs)
{

//  cout << "Inside initHistos Method" << endl;
...
// Track quality cuts
bool ecalReconstructedPi0::TrackQualityCuts(const reco::Track & track, const reco::Vertex & vertexCollectionSelected)
{

//  cout << "Inside TrackQualityCuts method" << endl;
...
bool ecalReconstructedPi0::vtxSort( const reco::Vertex &  a, const reco::Vertex & b )
{
//  cout << "Inside vtxSort method" << endl;
...
void ecalReconstructedPi0::convxtalid(Int_t &nphi,Int_t &neta) 
{
  // Barrel only
...
int ecalReconstructedPi0::diff_neta_s(Int_t neta1, Int_t neta2)
{
  Int_t mdiff;
  mdiff=(neta1-neta2);
  return mdiff;
...
int ecalReconstructedPi0::diff_nphi_s(Int_t nphi1,Int_t nphi2)
{
  Int_t mdiff;
...
bool ecalReconstructedPi0::printChildren(const SimTrack* p, 
...
float ecalReconstructedPi0::efficiencyCorrections(double pTtrig, double eta_trg, int tHighPurityTracks_) 
{
...
float ecalReconstructedPi0::crystalCorrectionFunction(int indexCrystal, float ecalEnergy)
{
  ...
  //                                                                                                                        
  //
//	cout << "inside crystalCorrectionFunction method" << endl;
...
bool ecalReconstructedPi0::pruneKids(const SimTrack* p,
...
DEFINE_FWK_MODULE(ecalReconstructedPi0);
